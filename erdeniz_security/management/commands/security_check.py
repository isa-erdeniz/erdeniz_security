...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        score = self._score(r)
        self.stdout.write("╔══════════════════════════════════════════╗")
        self.stdout.write("║     ErdenizTech Security Audit Report     ║")
        self.stdout.write(f"║     Project: {project_name[:28]:<28} ║")
        self.stdout.write("╠══════════════════════════════════════════╣")
        self.stdout.write(f"║  CRITICAL:  {r['critical']['pass']} passed / {r['critical']['fail']} failed")
        self.stdout.write(f"║  HIGH:      {r['high']['pass']} passed / {r['high']['fail']} failed")
        self.stdout.write(f"║  MEDIUM:    {r['medium']['pass']} passed / {r['medium']['fail']} failed")
        self.stdout.write(f"║  INFO:      {r['info']['pass']} passed / {r['info']['fail']} failed")
        self.stdout.write("╠══════════════════════════════════════════╣")
        label = "EXCELLENT" if score >= 90 else "GOOD" if score >= 70 else "NEEDS WORK"
        self.stdout.write(f"║  Score: {score}/100  {label}")
        self.stdout.write("╚══════════════════════════════════════════╝")
        for msg in failed:
